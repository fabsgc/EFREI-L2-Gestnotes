				...
				?>
						<td>
							<if condition="markEnabled($type,$course)">
								...
								<if condition="!reportNotGenerated($reports, $data['id'])">disabled</if> />
							<else/>
	...
	function updateMark(event, object, student, type)
	{
		...
		if(key==13){
			if(object.value != "")
			{
				$.ajax({
					type: "GET",
					url: "/teacher/mark/set/{$group['id']}/{$course['id']}/" + student + "/" + object.value + "/" + type + "/{$session['token']}",
					data: {
					}
				}).done(function(data) {
...
<?php
	...
	function reportNotGenerated($reports, $student)
	{
		foreach ($reports as $key => $value) {
			if($value['student']['id'] == $student)
			{
				return false;
			}
		}
